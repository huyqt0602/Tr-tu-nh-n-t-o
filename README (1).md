...
- Hỗ trợ **13 thuật toán** tìm kiếm thuộc 3 nhóm: Tìm kiếm mù, Tìm kiếm có thông tin, Tìm kiếm cục bộ
...
│   ├── local_beam_search.py               # Tìm kiếm chùm cục bộ (Local Beam Search)
...
| Leo đồi khởi động lại ngẫu nhiên (Random Restart HC) | Thực hiện leo đồi nhiều lần (tối đa 20 lần) từ các điểm xuất phát ngẫu nhiên khác nhau. Nếu một lần chạy đạt đích, kết nối đường đi bằng BFS. Trả về đường đi tốt nhất tìm được. |
| Tìm kiếm chùm cục bộ (Local Beam Search) | Duy trì đồng thời **k chùm** (k=3 mặc định) trạng thái. Tại mỗi bước, sinh toàn bộ lân cận của k trạng thái hiện tại, sau đó chọn ra k trạng thái tốt nhất để tiếp tục. |
| Mô phỏng luyện kim (Simulated Annealing) | Khởi đầu với nhiệt độ `T₀`, chấp nhận trạng thái lân cận tệ hơn với xác suất `e^(−Δ/T)`. Nhiệt độ giảm dần theo hệ số `α` (alpha=0.98) qua mỗi bước cho đến khi `T < Tₘᵢₙ`. |

> **Lưu ý về Local Search:** Các thuật toán leo đồi không đảm bảo tìm được đích trong mọi trường hợp. Khi bị kẹt ở cực đại cục bộ (do vật cản), robot sẽ hiển thị đường đi đến vị trí tốt nhất đã đạt được. Simulated Annealing và Local Beam Search có khả năng thoát khỏi cực đại cục bộ tốt hơn so với các biến thể leo đồi cơ bản.
...
cd Tr-tu-nh-n-t-o/vaccum_cleaner
